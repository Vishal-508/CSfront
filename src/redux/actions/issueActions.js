...
export const fetchUserIssues = () => async (dispatch) => {
  try {
    ...
    const { data } = await API.get('/api/issues/user');
    dispatch({
...
// Create new issue
export const createIssue = (formData) => async (dispatch) => {
  try {
...
export const updateIssue = ({ id, issueData }) => async (dispatch) => {
  try {
    ...
    Object.keys(issueData).forEach(key => {
      if (issueData[key] !== null) {
        formData.append(key, issueData[key]);
      }
    });
    
    const { data } = await API.patch(`/api/issues/${id}`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    });
